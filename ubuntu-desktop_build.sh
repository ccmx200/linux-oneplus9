...
# 挂载boot
mount -o loop oneplus-lemonade-boot.img rootdir/boot
...
127.0.1.1 oneplus-lemonade" | tee rootdir/etc/hosts
...
# 复制并安装内核包（从预下载的目录）
cp oneplus-lemonade-debs_$2/*-oneplus-lemonade.deb rootdir/tmp/
chroot rootdir dpkg -i /tmp/linux-image-oneplus-lemonade.deb
chroot rootdir dpkg -i /tmp/linux-headers-oneplus-lemonade.deb
chroot rootdir dpkg -i /tmp/firmware-oneplus-lemonade.deb
chroot rootdir dpkg -i /tmp/alsa-oneplus-lemonade.deb
rm rootdir/tmp/*-oneplus-lemonade.deb
...
echo oneplus-lemonade > $G/strings/0x409/manufacturer
